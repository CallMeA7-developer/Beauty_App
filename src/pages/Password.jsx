  ...
  return (
    <div className="bg-white font-['Cormorant_Garamond']">
      ...
      <div className="min-h-[48px] bg-[#FDFBF7] px-[120px] flex items-center">
        ...
        <Link to="/dashboard"><span className="text-[15px] font-normal text-[#8B7355] cursor-pointer">Account Dashboard</span></Link>
        <span className="text-[15px] font-normal text-[#666666] mx-2">/</span>
          ...
          <div className="flex-1">
            ...
            <div className="bg-white rounded-[12px] shadow-[0_4px_16px_rgba(0,0,0,0.08)] p-[32px] mb-[24px]">
              ...
              <div className="space-y-[20px]">
                ...
                {/* Current Password */}
                <div>
                  <label className="text-[16px] font-medium text-[#1A1A1A] block mb-[12px]">Current Password</label>
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-white border-[1.5px] border-[#E8E3D9] rounded-[8px] px-[16px] h-[56px] text-[15px] font-normal text-[#1A1A1A] outline-none focus:border-[#8B7355] transition-colors pr-[48px]"
                    />
                    <button onClick={() => setShowCurrent(!showCurrent)} className="absolute right-[16px] top-1/2 -translate-y-1/2 cursor-pointer">
                      {showCurrent ? <IoEyeOffOutline className="w-[20px] h-[20px] text-[#666666]" /> : <IoEyeOutline className="w-[20px] h-[20px] text-[#666666]" />}
                    </button>
                  </div>
                  <span className="text-[13px] font-normal text-[#8B7355] underline cursor-pointer hover:text-[#7a6448] transition-colors inline-block mt-[8px]">
                    Forgot password?
                  </span>
                </div>
                ...
                {/* New Password */}
                <div>
                  <label className="text-[16px] font-medium text-[#1A1A1A] block mb-[12px]">New Password</label>
                  <div className="relative">
                    <input
                    ...
                    <button onClick={() => setShowNew(!showNew)} className="absolute right-[16px] top-1/2 -translate-y-1/2 cursor-pointer">
                      {showNew ? <IoEyeOffOutline className="w-[20px] h-[20px] text-[#666666]" /> : <IoEyeOutline className="w-[20px] h-[20px] text-[#666666]" />}
                    </button>
                  </div>
                  {newPass.length > 0 && (
                    <div className="mt-[10px]">
                      ...
                      <div className="flex justify-end mt-[4px]">
                        <span className={`text-[13px] font-medium ${strength.color}`}>Strength: {strength.label}</span>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {/* Confirm Password */}
                <div>
                  <label className="text-[16px] font-medium text-[#1A1A1A] block mb-[12px]">Confirm New Password</label>
                  <div className="relative">
                    <input
                    ...
                    <button onClick={() => setShowConfirm(!showConfirm)} className="absolute right-[16px] top-1/2 -translate-y-1/2 cursor-pointer">
                      {showConfirm ? <IoEyeOffOutline className="w-[20px] h-[20px] text-[#666666]" /> : <IoEyeOutline className="w-[20px] h-[20px] text-[#666666]" />}
                    </button>
                  </div>
                  {confirm.length > 0 && newPass !== confirm && (
                    <p className="text-[13px] text-red-500 mt-[6px]">Passwords do not match</p>
                  )}
                  {confirm.length > 0 && newPass === confirm && (
                    <p className="text-[13px] text-green-600 mt-[6px] flex items-center gap-1">
                      <IoCheckmarkCircle className="w-[14px] h-[14px]" /> Passwords match
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-[10px]">
                {requirements.map((req, idx) => (
                  <div key={idx} className="flex items-center gap-[12px]">
                    <IoCheckmarkCircle className={`w-[16px] h-[16px] flex-shrink-0 transition-colors ${req.met ? 'text-green-600' : 'text-[#E8E3D9]'}`} />
                    <span className={`text-[14px] font-normal transition-colors ${req.met ? 'text-[#1A1A1A]' : 'text-[#999999]'}`}>{req.text}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex gap-[16px] mb-[24px]">
              <Link to="/dashboard" className="flex-1">
                <button className="w-full h-[56px] bg-white border-[1.5px] border-[#E8E3D9] text-[#666666] text-[16px] font-medium rounded-[8px] cursor-pointer hover:border-[#8B7355] hover:text-[#8B7355] transition-all">
                  Cancel
                </button>
              </Link>
              <button className="flex-1 h-[56px] bg-[#8B7355] text-white text-[16px] font-medium rounded-[8px] cursor-pointer hover:bg-[#7a6448] transition-colors flex items-center justify-center gap-2">
            ...
            <div className="bg-[#FDFBF7] rounded-[12px] p-[20px] flex items-center gap-[12px] mb-[24px]">
              ...
              <span className="text-[14px] font-normal text-[#666666]">Last password change: November 15, 2024</span>
            </div>
