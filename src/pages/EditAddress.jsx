...
export default function EditAddress() {
  ...
  const navigationItems = getNavItems('settings', NAV_ICONS).map((item) => ({
    ...item,
    label: t(`editAddress.navItems.${item.label}`, { defaultValue: item.label }),
    badge: item.badge
      ? t(`editAddress.badges.${item.badge}`, { defaultValue: item.badge })
      : item.badge,
    tag: item.tag
      ? t(`editAddress.tags.${item.tag}`, { defaultValue: item.tag })
      : item.tag,
  }))
  ...
  const translatedCountries = COUNTRY_OPTIONS.map((country) => ({
    value: country,
    label: t(`editAddress.countries.${country}`, { defaultValue: country }),
  }))

  const quickStats = [
    { label: t('editAddress.totalOrders'), value: '24' },
    { label: t('editAddress.wishlistItems'), value: '12' },
    { label: t('editAddress.reviewsWritten'), value: '8' },
  ]
      ...
      >
        ...
        <Link to="/shipping-address">
          <span className="hidden md:inline text-[13px] lg:text-[15px] font-normal text-[#8B7355] cursor-pointer whitespace-nowrap">
            {t('editAddress.shippingAddresses')}
          </span>
        </Link>
            ...
            <div className="bg-white rounded-[12px] shadow-[0_4px_16px_rgba(0,0,0,0.08)] p-5 lg:p-[28px] mb-5 lg:mb-[24px]">
              <div className="flex flex-col items-center">
                <img
                  ...
                  alt={t('editAddress.userAvatarAlt')}
          ...
          <div className="flex-1 min-w-0">
            <div className="bg-white rounded-[12px] shadow-[0_4px_16px_rgba(0,0,0,0.08)] p-5 md:p-6 lg:p-[40px]">
              <div className="space-y-4 lg:space-y-[24px]">
                {/* Address Label */}
                <div>
                  <label className={labelClass}>{t('editAddress.addressLabel')}</label>
                  <input
                    type="text"
                    value={form.label}
                ...
                <div>
                  <label className={labelClass}>
                    {t('editAddress.fullName')} <span className="text-[#C9A870]">*</span>
                  </label>
                  <input
                    type="text"
                    value={form.fullName}
                ...
                <div>
                  <label className={labelClass}>
                    {t('editAddress.phoneNumber')} <span className="text-[#C9A870]">*</span>
                  </label>
                ...
                <div>
                  <label className={labelClass}>
                    {t('editAddress.addressLine1')} <span className="text-[#C9A870]">*</span>
                  </label>
                  <input
                    type="text"
                    value={form.address1}
                ...
                <div>
                  <label className="text-[13px] lg:text-[14px] font-medium text-[#666666] block mb-[6px] lg:mb-[8px]">
                    {t('editAddress.addressLine2')}{' '}
                    <span className="font-light">({t('editAddress.optional')})</span>
                  </label>
                  <input
                    type="text"
                    value={form.address2}
                    onChange={(e) => update('address2', e.target.value)}
                    placeholder={t('editAddress.addressLine2Placeholder')}
                    className={inputClass}
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-[60%_1fr] gap-4 lg:gap-[16px]">
                  <div>
                    <label className={labelClass}>
                      {t('editAddress.city')} <span className="text-[#C9A870]">*</span>
                    </label>
                    <input
                      type="text"
                      value={form.city}
                  ...
                  <div>
                    <label className={labelClass}>
                      {t('editAddress.stateRegion')} <span className="text-[#C9A870]">*</span>
                    </label>
                    <div className="relative">
                      <input
                        type="text"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-[40%_1fr] gap-4 lg:gap-[16px]">
                  <div>
                    <label className={labelClass}>
                      {t('editAddress.postalCode')} <span className="text-[#C9A870]">*</span>
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className={labelClass}>
                      {t('editAddress.country')} <span className="text-[#C9A870]">*</span>
                    </label>
                    <div className="relative">
                      <select
                        value={form.country}
