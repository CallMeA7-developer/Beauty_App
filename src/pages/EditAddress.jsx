...
const useCountryTranslation = () => {
  ...
  const tf = (value) => {
    if (i18n.language === 'ru') {
      return countryTranslationsRU[value] || value
    }
    return value
  }
...
export default function EditAddress() {
  ...
  const navigationItems = getNavItems('settings', NAV_ICONS).map((item) => ({
    ...item,
    label: navLabelKeyMap[item.label]
      ? t(`editAddress.sidebarNav.${navLabelKeyMap[item.label]}`)
      : item.label,
    badge:
      item.badge === '12'
        ? t('editAddress.sidebarNav.badgeWishlist')
        : item.badge,
    tag: item.tag && navTagKeyMap[item.tag]
      ? t(`editAddress.sidebarNav.tags.${navTagKeyMap[item.tag]}`)
      : item.tag,
  }))

  const tfCountry = useCountryTranslation()
  ...
  const inputClass =
    "w-full h-[48px] lg:h-[56px] border border-[#E8E3D9] rounded-[8px] px-[16px] text-[14px] lg:text-[15px] font-normal text-[#1A1A1A] outline-none focus:border-[#8B7355] transition-colors"
  const labelClass =
    "text-[13px] lg:text-[14px] font-medium text-[#1A1A1A] block mb-[6px] lg:mb-[8px]"

  const stats = [
    { label: t('editAddress.quickStats.totalOrders'), value: '24' },
    { label: t('editAddress.quickStats.wishlistItems'), value: '12' },
    { label: t('editAddress.quickStats.reviewsWritten'), value: '8' },
  ]
      ...
      >
        ...
        <Link to="/dashboard">
          <span className="hidden sm:inline text-[13px] lg:text-[15px] font-normal text-[#8B7355] cursor-pointer whitespace-nowrap">
            {t('editAddress.breadcrumb.account')}
          </span>
        </Link>
        <span className="hidden sm:inline text-[13px] lg:text-[15px] font-normal text-[#666666] mx-2">/</span>

        <Link to="/shipping-address">
          <span className="hidden md:inline text-[13px] lg:text-[15px] font-normal text-[#8B7355] cursor-pointer whitespace-nowrap">
            {t('editAddress.breadcrumb.shippingAddresses')}
          </span>
        </Link>
        ...
        <span className="text-[13px] lg:text-[15px] font-normal text-[#666666] whitespace-nowrap">
          {t('editAddress.breadcrumb.editAddress')}
        </span>
      </div>
            ...
            <div className="bg-white rounded-[12px] shadow-[0_4px_16px_rgba(0,0,0,0.08)] p-5 lg:p-[28px] mb-5 lg:mb-[24px]">
              <div className="flex flex-col items-center">
                <img
                  ...
                  alt={t('editAddress.profile.avatarAlt')}
                ...
                <div className="bg-[#C9A870] text-white text-[11px] lg:text-[12px] font-medium px-[14px] lg:px-[16px] py-[5px] lg:py-[6px] rounded-full mb-4 lg:mb-[16px]">
                  {t('editAddress.profile.memberTier')}
                </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="bg-white rounded-[12px] shadow-[0_4px_16px_rgba(0,0,0,0.08)] p-5 md:p-6 lg:p-[40px]">
              <div className="space-y-4 lg:space-y-[24px]">
                {/* Address Label */}
                <div>
                  <label className={labelClass}>{t('editAddress.form.addressLabel')}</label>
                  <input
                    type="text"
                    value={form.label}
                ...
                <div>
                  <label className={labelClass}>
                    {t('editAddress.form.fullName')} <span className="text-[#C9A870]">*</span>
                  </label>
                  <input
                    type="text"
                    value={form.fullName}
                ...
                <div>
                  <label className={labelClass}>
                    {t('editAddress.form.phoneNumber')} <span className="text-[#C9A870]">*</span>
                  </label>
                ...
                <div>
                  <label className={labelClass}>
                    {t('editAddress.form.addressLine1')} <span className="text-[#C9A870]">*</span>
                  </label>
                  <input
                    type="text"
                    value={form.address1}
                ...
                <div>
                  <label className="text-[13px] lg:text-[14px] font-medium text-[#666666] block mb-[6px] lg:mb-[8px]">
                    {t('editAddress.form.addressLine2')}{' '}
                    <span className="font-light">({t('editAddress.form.optional')})</span>
                  </label>
                  <input
                    type="text"
                    value={form.address2}
                    onChange={(e) => update('address2', e.target.value)}
                    placeholder={t('editAddress.form.addressLine2Placeholder')}
                    className={inputClass}
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-[60%_1fr] gap-4 lg:gap-[16px]">
                  <div>
                    <label className={labelClass}>
                      {t('editAddress.form.city')} <span className="text-[#C9A870]">*</span>
                    </label>
                    <input
                      type="text"
                      value={form.city}
                  ...
                  <div>
                    <label className={labelClass}>
                      {t('editAddress.form.stateRegion')} <span className="text-[#C9A870]">*</span>
                    </label>
                    <div className="relative">
                      <input
                        type="text"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-[40%_1fr] gap-4 lg:gap-[16px]">
                  <div>
                    <label className={labelClass}>
                      {t('editAddress.form.postalCode')} <span className="text-[#C9A870]">*</span>
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className={labelClass}>
                      {t('editAddress.form.country')} <span className="text-[#C9A870]">*</span>
                    </label>
                    <div className="relative">
                      <select
                        value={form.country}
                ...
                >
                  <div
                    className={`w-[18px] h-[18px] lg:w-[20px] lg:h-[20px] border-2 rounded-[4px] flex items-center justify-center flex-shrink-0 transition-colors ${
                      form.isDefault
                  ...
                  >
                    {form.isDefault && (
                      <IoCheckmark className="w-[12px] h-[12px] lg:w-[14px] lg:h-[14px] text-white" />
                    )}
                  </div>
                  <span className="text-[13px] lg:text-[15px] font-normal text-[#3D3D3D] select-none">
                    {t('editAddress.form.setAsDefault')}
                  </span>
                </div>
              </div>
              ...
              <div className="border-t border-[#E8E3D9] mt-6 lg:mt-[32px] pt-5 lg:pt-[32px]">
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 lg:gap-[16px]">
                  ...
                  <button className="w-full sm:w-auto bg-[#8B7355] text-white text-[14px] lg:text-[15px] font-medium px-6 lg:px-[40px] py-[12px] lg:py-[14px] rounded-[8px] flex items-center justify-center gap-[8px] cursor-pointer hover:bg-[#7a6448] transition-colors">
                    ...
                    {t('editAddress.actions.saveAddress')}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 lg:mt-[32px] bg-gradient-to-b from-[#F5F1EA] to-white rounded-[12px] p-5 lg:p-[28px] flex items-center gap-4 lg:gap-[16px]">
              ...
              <div>
                <h4 className="text-[15px] lg:text-[18px] font-medium text-[#1A1A1A] mb-[6px]">
                  {t('editAddress.security.title')}
                </h4>
                <p className="text-[12px] lg:text-[14px] font-normal text-[#666666] leading-[1.6]">
                  {t('editAddress.security.description')}
                </p>
              </div>
            </div>
          </div>
