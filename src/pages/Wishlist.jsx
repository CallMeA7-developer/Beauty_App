          ...
          {sortOptions.map((option, idx) => (
            <div key={option.id}>
              <button
                ...
                className={`w-full min-h-[56px] px-5 flex items-center justify-between ${
                  selected === option.label ? 'bg-[#FDFBF7]' : ''
                }`}
              >
                <div className="flex items-center gap-3">
                  <option.icon className={`w-[18px] h-[18px] ${
                    selected === option.label ? 'text-[#C9A870]' : 'text-[#666666]'
                  }`} />
                  <span className={`text-[15px] ${
                    selected === option.label ? 'text-[#8B7355] font-medium' : 'text-[#2B2B2B] font-normal'
                  }`}>
                    {option.label}
                  </span>
                </div>
                {selected === option.label && (
                  <IoCheckmark className="w-5 h-5 text-[#C9A870]" />
                )}
              </button>
              {idx < sortOptions.length - 1 && (
                <div className="h-[1px] bg-[#F5F1EA] mx-5" />
              )}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="w-full min-h-screen bg-[#FDFBF7] font-['Cormorant_Garamond'] flex flex-col">
      ...
      <div className="bg-white border-b border-[#E8E3D9] px-5 h-[60px] flex items-center justify-between flex-shrink-0">
        ...
        <div className="w-11 h-11 flex items-center justify-center">
          <IoShareSocialOutline className="w-6 h-6 text-[#2B2B2B]" />
        </div>
      </div>
      ...
      <div className="bg-white shadow-[0_2px_8px_rgba(0,0,0,0.04)] px-5 py-3 flex items-center justify-between gap-3 flex-shrink-0">
        <button
          ...
          className="h-10 px-4 border-[1.5px] border-[#E8E3D9] rounded-lg flex items-center gap-2"
        >
        ...
        <button className="h-10 px-4 border-[1.5px] border-[#E8E3D9] rounded-lg flex items-center gap-2">
          <IoFunnelOutline className="w-4 h-4 text-[#666666]" />
          <span className="text-[14px] font-medium text-[#666666]">Filter</span>
        </button>
      </div>
        ...
        {wishlistProducts.map((product) => (
          <div key={product.id} className="bg-white rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-4">
            ...
            <div className="mb-4">
              ...
              <div className={`inline-block px-2.5 py-1 rounded text-[12px] font-medium ${
        ...
        <div className="max-w-[1200px] mx-auto flex gap-[40px]">
          ...
          <div className="flex-1">
              ...
              {wishlistProducts.map((product) => (
                <div key={product.id} className="bg-white rounded-[12px] shadow-[0_4px_16px_rgba(0,0,0,0.08)] p-[20px] hover:shadow-[0_8px_32px_rgba(0,0,0,0.12)] transition-all duration-300">
                  <div className="relative w-full h-[280px] rounded-[8px] overflow-hidden mb-[16px] group">
                    ...
                    {product.stock === 'Low Stock' && (
                      <div className="absolute top-[12px] left-[12px] bg-[#E5A84D] text-white text-[11px] font-medium px-[10px] py-[4px] rounded-full">Low Stock</div>
                    )}
                  </div>
                  <div className="mb-[16px]">
                    ...
                    <div className="flex items-center gap-[8px] mb-[8px]">
                      <div className="flex items-center gap-[2px]">
                        {[1,2,3,4,5].map((s) => <IoStarSharp key={s} className="w-[14px] h-[14px] text-[#C9A870]" />)}
                      </div>
                      <span className="text-[13px] text-[#999999]">({product.reviews})</span>
                    </div>
                    <div className="flex items-center gap-[8px] mb-[8px]">
                      {product.originalPrice !== product.currentPrice && (
                        <span className="text-[16px] text-[#999999] line-through">${product.originalPrice}</span>
                      )}
                      <span className="text-[22px] font-semibold text-[#1A1A1A]">${product.currentPrice}</span>
                      {product.originalPrice !== product.currentPrice && (
            ...
            <div className="bg-[#F5F1EA] rounded-[12px] p-[24px] flex items-center justify-between mb-[32px]">
              ...
              <button className="h-[48px] px-[32px] bg-white border-[1.5px] border-[#E8E3D9] text-[#666666] text-[15px] font-medium rounded-[8px] hover:border-[#8B7355] hover:text-[#8B7355] transition-all">
                Continue Shopping
              </button>
            </div>
