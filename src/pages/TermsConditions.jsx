...
export default function TermsConditions() {
  ...
  const sections = t('termsConditions.sections', { returnObjects: true })
  ...
  return (
    <div key={i18n.language} className="bg-[#FDFBF7] font-['Cormorant_Garamond'] min-h-screen">
      ...
      <div className="min-h-[48px] bg-white px-4 md:px-[60px] lg:px-[120px] flex items-center border-b border-[#E8E3D9]">
        <Link to="/"><span className="text-[#8B7355]">{t('termsConditions.breadcrumb.home')}</span></Link>
        <span className="mx-2">/</span>
        <span>{t('termsConditions.breadcrumb.current')}</span>
      </div>
        ...
        <div className="max-w-[860px] mx-auto">
          ...
          <div className="bg-white rounded-[16px] border p-6 mb-8">
            <p className="text-[#666]">
              {t('termsConditions.intro.part1')}
              <span className="font-semibold text-[#8B7355]">Shan Loray</span>
              {t('termsConditions.intro.part2')}
            </p>

            <p className="text-[#666] mt-4">
              {t('termsConditions.intro.part3')}
            </p>
          </div>
          ...
          <div className="space-y-4 mb-12">
            {sections.map((section, sIndex) => (
              <div key={sIndex} id={sIndex === 3 ? 'returns' : undefined} className="bg-white rounded-[12px] border overflow-hidden">

                <button onClick={() => toggle(sIndex)} className="w-full flex justify-between px-6 py-5">
                  <span className="font-semibold">{section.title}</span>
                  <IoChevronDown className={`${openSections[sIndex] ? 'rotate-180' : ''}`} />
                </button>
          ...
          <div className="bg-[#F5F1EA] rounded-[16px] p-6 text-center">
            <h3 className="text-[22px] font-semibold mb-2">
              {t('termsConditions.footer.title')}
            </h3>

            <p className="text-[#666] mb-6">
              {t('termsConditions.footer.desc')}
            </p>

            <div className="flex gap-3 justify-center">
              <Link to="/privacy-policy">
                <button className="border px-6 py-3">
                  {t('termsConditions.footer.privacy')}
                </button>
              </Link>

              <Link to="/faq">
                <button className="bg-[#8B7355] text-white px-6 py-3">
                  {t('termsConditions.footer.faq')}
                </button>
              </Link>
            </div>
          </div>
