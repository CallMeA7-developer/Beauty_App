...
export default function Filter() {
  ...
  return (
    <div className="min-h-screen bg-[#1A1A1A]/40 flex items-center justify-center font-['Cormorant_Garamond'] py-[40px]">
      <div className="w-[560px] bg-white rounded-[16px] shadow-[0_8px_40px_rgba(0,0,0,0.15)] flex flex-col max-h-[90vh]">

        {/* ── Header ── */}
        <div className="min-h-[72px] px-[24px] flex items-center justify-between border-b-[1.5px] border-[#E8E3D9] flex-shrink-0">
          <h1 className="text-[28px] font-semibold text-[#1A1A1A]">Filter Products</h1>
          <button onClick={() => navigate(-1)}>
            <IoCloseOutline className="w-[28px] h-[28px] text-[#666666] cursor-pointer hover:text-[#1A1A1A] transition-colors" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-[32px] py-[32px]">
          ...
          <div className="mb-[28px]">
            <h3 className="text-[17px] font-medium text-[#1A1A1A] mb-[16px]">Product Type</h3>
          ...
          {/* Price Range */}
          <div className="mb-[28px]">
            <h3 className="text-[17px] font-medium text-[#1A1A1A] mb-[16px]">Price Range</h3>
            ...
            <div className="text-center">
              <span className="text-[15px] font-normal text-[#666666]">$0 — $500</span>
            </div>
          </div>

          {/* Skin Type */}
          <div className="mb-[28px]">
            <h3 className="text-[17px] font-medium text-[#1A1A1A] mb-[16px]">Skin Type</h3>
            <div className="space-y-[12px]">
              {skinTypes.map((type) => (
                <div key={type} className="flex items-center gap-[12px]" onClick={() => toggle(selectedSkinTypes, setSelectedSkinTypes, type)}>
                  ...
                  <span className="text-[15px] font-normal text-[#2B2B2B] cursor-pointer select-none">{type}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Skin Concerns */}
          <div className="mb-[28px]">
            <h3 className="text-[17px] font-medium text-[#1A1A1A] mb-[16px]">Skin Concerns</h3>
          ...
          {/* Brand */}
          <div className="mb-[28px]">
            <h3 className="text-[17px] font-medium text-[#1A1A1A] mb-[12px]">Brand</h3>
            <div className="relative mb-[12px]">
              <input
                type="text"
                placeholder="Search brands..."
                className="w-full h-[48px] px-[16px] pr-[44px] border-[1.5px] border-[#E8E3D9] rounded-[8px] text-[15px] font-light italic text-[#999999] outline-none focus:border-[#8B7355] transition-colors"
              />
              <IoSearchOutline className="absolute right-[16px] top-1/2 -translate-y-1/2 w-[20px] h-[20px] text-[#999999]" />
            </div>
            <div className="max-h-[180px] overflow-y-auto space-y-[8px] pr-1">
              {brands.map((brand) => (
                <div key={brand} className="h-[40px] flex items-center gap-[12px]" onClick={() => toggle(selectedBrands, setSelectedBrands, brand)}>
                  ...
                  <span className="text-[15px] font-normal text-[#2B2B2B] cursor-pointer select-none">{brand}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Product Benefits */}
          <div className="mb-[28px]">
            <h3 className="text-[17px] font-medium text-[#1A1A1A] mb-[16px]">Product Benefits</h3>
            <div className="flex flex-wrap gap-[8px]">
          ...
          {/* Minimum Rating */}
          <div className="mb-[28px]">
            <h3 className="text-[17px] font-medium text-[#1A1A1A] mb-[16px]">Minimum Rating</h3>
              ...
              <div className="flex items-center gap-[6px]">
                {[1, 2, 3, 4, 5].map((star) => (
                  <IoStarSharp
                    ...
                    className={`w-[28px] h-[28px] cursor-pointer transition-colors ${star <= minRating ? 'text-[#C9A870]' : 'text-[#E8E3D9]'}`}
                  />
                ))}
              </div>
          ...
          <div className="mb-[32px]">
            <h3 className="text-[17px] font-medium text-[#1A1A1A] mb-[16px]">Availability</h3>
            <div className="space-y-[12px]">
              <div className="flex items-center gap-[12px]" onClick={() => setInStockOnly(true)}>
                <Radio checked={inStockOnly} />
                <span className="text-[15px] font-normal text-[#2B2B2B] cursor-pointer select-none">In Stock</span>
              </div>
              <div className="flex items-center gap-[12px]" onClick={() => setInStockOnly(false)}>
                <Radio checked={!inStockOnly} />
                <span className="text-[15px] font-normal text-[#2B2B2B] cursor-pointer select-none">Include Out of Stock</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* ── Footer ── */}
        <div className="min-h-[88px] bg-[#FDFBF7] px-[32px] py-[24px] border-t-[1.5px] border-[#E8E3D9] flex items-center gap-[16px] flex-shrink-0 rounded-b-[16px]">
          <button
            onClick={() => {
              setSelectedTypes([])
              setSelectedConcerns([])
            ...
            className="flex-1 h-[52px] bg-white border-[1.5px] border-[#E8E3D9] text-[#666666] text-[16px] font-medium rounded-[8px] cursor-pointer hover:border-[#8B7355] hover:text-[#8B7355] transition-all"
          >
            Clear Filters
          </button>
          <button
            onClick={() => navigate(-1)}
            className="flex-1 h-[52px] bg-[#8B7355] text-white text-[16px] font-medium rounded-[8px] cursor-pointer hover:bg-[#7a6448] transition-colors"
          >
            Apply Filters
          </button>
        </div>
